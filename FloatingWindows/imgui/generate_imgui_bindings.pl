  ...
  foreach $line (split /\n/, $imguiCodeBlock) {
    ...
    if ($line =~ m/$lineCaptureRegex/) {
      ...
      for (my $i = 0; $i < @args; $i++) {
        ...
        } elsif ($args[$i] =~ m/^ *(ImGuiWindowFlags|ImGuiCol|ImGuiStyleVar|ImGuiKey|ImGuiAlign|ImGuiColorEditMode|ImGuiMouseCursor|ImGuiSetCond|ImGuiCond|ImGuiInputTextFlags|ImGuiSelectableFlags) ([^ ]*)( = 0|) *$/) {
         #These are ints
         my $name = $2;
          if ($3 =~ m/^ = 0$/) {
