-- This script is identical to floating_wnd_test.lua in almost every way, except the is passes callback
-- functions for floating windows by reference, rather than name. This allows using local or even anonymous functions
-- for managing floating windows.
...
local lastClickX = 640 / 2
local lastClickY = 480 / 2

-- x and y are the origin of the window, i.e. the lower left
-- x increases to the right, y increases to the top
local function on_draw(wnd, x, y)
    local centerX = x + lastClickX
    local centerY = y + lastClickY
...
local function on_click(wnd, x, y, state)
    lastClickX = x
    lastClickY = y
end
...
local function on_close(wnd)
    XPLMSpeakString("Bye, see you later!")
end
